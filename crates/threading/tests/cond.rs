#[cfg(test)]
mod tests {
    use emixthreading::{AutoResetCond, CountdownCond, ManualResetCond};
    use std::sync::{
        Arc,
    ...
    fn test_autoreset_new_set() {
        ...
        assert!(
            signaled.unwrap(),
            "Should be signaled when created with new_set()"
        );
    }
    ...
    fn test_autoreset_new_with_state() {
        ...
        assert!(
            cond.is_set().unwrap(),
            "Should be set when created with true"
        );

        let cond = AutoResetCond::new(false);
        assert!(
            !cond.is_set().unwrap(),
            "Should not be set when created with false"
        );
    }
    ...
    fn test_autoreset_wait_auto_resets() {
        ...
        assert!(
            !signaled.unwrap(),
            "Flag should be reset after wait (auto-reset behavior)"
        );
    }
    ...
    fn test_autoreset_only_one_waiter_wakes() {
        ...
        assert_eq!(
            woke_count.load(Ordering::SeqCst),
            1,
            "Exactly one waiter should have woken"
        );
        ...
        assert_eq!(
            woke_count.load(Ordering::SeqCst),
            2,
            "Exactly two waiters should have woken"
        );
    ...
    fn test_autoreset_wait_timeout_while_condition_met() {
        ...
        let flag_ref = flag.clone();
        let result = cond.wait_timeout_while(
            || flag_ref.load(Ordering::SeqCst) < 5,
            Duration::from_millis(200),
        );
        assert!(result.is_ok(), "wait_timeout_while should succeed");
        assert!(
            result.unwrap(),
            "Should not timeout, condition should be met"
        );
    }
    ...
    fn test_autoreset_concurrent_sets() {
        ...
        assert!(
            cond.is_set().unwrap(),
            "Should be set after concurrent sets"
        );
    }
    ...
    fn test_manualreset_wait_does_not_reset() {
        ...
        assert!(
            signaled.unwrap(),
            "Flag should remain set after wait (manual reset behavior)"
        );
    }
    ...
    fn test_manualreset_multiple_waits() {
        ...
        assert!(
            cond.is_set().unwrap(),
            "Should still be set after multiple waits"
        );
    }
    ...
    fn test_manualreset_all_waiters_wake() {
        ...
        assert!(
            signaled.unwrap(),
            "Flag should remain true after set() wakes all waiters"
        );
    }
    ...
    fn test_countdown_add_count_zero() {
        ...
        assert!(
            !cond.is_set().unwrap(),
            "Should not be set after adding count"
        );
    }
        ...
        for i in 0..5 {
            ...
            if i < 4 {
                assert_eq!(
                    woke_count.load(Ordering::SeqCst),
                    0,
                    "No waiters should wake until count reaches 0"
                );
            }
        }
